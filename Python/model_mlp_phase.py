...
MLP on differential phase features — shape (N, WINDOW=11).
Rotation-invariant: unaffected by constant or slowly-varying phase offsets.

Hyperparameter tuning:
    Random search, same protocol as model_mlp_iq.py.

Search space:
    hidden_dims : [(32,16), (64,32), (128,64), (128,64,32), (256,128,64)]
...
from common import (
    ...
    make_features_phase, MLP, train_mlp, predict_mlp,
...
def run():
    print("\n" + "=" * 60)
    print("MLP-Phase  (differential phase window, rotation-invariant)")
    print("=" * 60)
    ...
    X_tr_full, y_tr_full, _ = build_dataset(pairs_tr, make_features_phase)
    X_te,      y_te,      l_te = build_dataset(pairs_te, make_features_phase)
    ...
    final_val_ber, scaler = train_mlp(
        ...
        epochs=FINAL_EPOCHS, name="MLP-Phase", verbose=True)
    ...
    save_path = os.path.join(MODEL_DIR, "mlp_phase.pt")
    torch.save({
